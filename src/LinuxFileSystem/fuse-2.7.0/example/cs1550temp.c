...
static void write_data_block(long _offset, struct cs1550_disk_block *data);
...
static int cs1550_getattr(const char *path, struct stat *stbuf)
{
	...
	struct cs1550_root_directory curr_root = read_root();
	int directory_num = curr_root.nDirectories;		// local number of directory
	...
	printf("\nNumber of Directory = %d\n"
			"Name of directory = %s \nDirectory block Num = %ld\n"
			, directory_num, curr_root.directories[directory_num].dname
			, curr_root.directories[directory_num - 1].nStartBlock);

	// struct cs1550_directory_entry test_entry = read_dir_entry(curr_root.lastAllocatedBlock);

	// printf("\nDirectory Entry::\nNumFiles = %d\nFilename = %s.%s\n", test_entry.nFiles, 
	// test_entry.files[test_entry.nFiles].fname,test_entry.files[test_entry.nFiles].fext);


	/* Check if the directory is a fresh new directory or subdirectory	*/

	// First look for the directory in the root
	int dir_index = 0;
	int i;
	printf("Looking into directory array... [");
	for(i = 0; i < directory_num; i++){
		if(strcmp(directory, curr_root.directories[i].dname) == 0){
			printf("%s", curr_root.directories[i].dname);
			dir_index = i;
			goto dir_found;
		}
	}
	// The directory do not exist so return error that 
	// the given directory does not exist as -ENOENT.
	printf("] Directory Not found...\n");
	return -ENOENT;


	dir_found:	// At this point, there is a given directory exist
		...
		// At this point, there is a file exist so we need to fetch that file information.

		struct cs1550_directory_entry entry = read_dir_entry(curr_root.directories[dir_index].nStartBlock);

		//Now search throught the file name in the given directory
		int file_index = 0;
		for(i = 0; i < entry.nFiles; i++){

			if(strcmp(filename, entry.files[i].fname) == 0 &&
				strcmp(extension, entry.files[i].fext) == 0){
					file_index = i;
				goto file_found;
			}
		}

		// The file does not exist in this line so it will return the error
		// message that there is no such directory or file as -ENOENT.
		printf("File does not exist...\n");
		return -ENOENT;


		file_found: 	// At this point, there is a file on the .disk so return the information about file finding
			 ...
			 off_t offset, struct fuse_file_info *fi)
{
	...
	int directory_num = root.nDirectories;			// numbers of total directory in the root directory
	...
	// At this point, directory name exist so we need to search through the root
	// directory structure (table). Once the directory name is found in the disk, 
	// then return the location of that directory such as offset. 
	long _offset = 0;
	for (i = 0; i < directory_num; i++)
	{
		if(strcmp(directory, root.directories[i].dname) == 0){
			_offset = root.directories[i].nStartBlock;
			goto directory_found;
		}
	}

	// At this point, there is no directory in the root directory table so return error
	return -ENOENT;

	directory_found:	// At this point, the directory exist in the root directory table

		// Fetch the diectory entry table from the found directory location
		struct cs1550_directory_entry entry = read_dir_entry(_offset);

		int total_file_number = entry.nFiles;		// total number of files

		// At this point, we need to look for the file in the directory entry table
		// And print them out in the current directory
		for(i = 0; i < total_file_number; i++){

			if(strcmp(entry.files[i].fname, "") != 0 
			&& strcmp(entry.files[i].fext, "") != 0){
				char *str = strcat(filename,".");
				str = strcat(str, extension);
...
static int cs1550_mkdir(const char *path, mode_t mode)
{
	...
	// 
	struct cs1550_root_directory root = read_root();
	int directory_num = root.nDirectories;		// total number of directories 
	long last_blocks = root.lastAllocatedBlock; // the offset or previous block location number

	// Check if the total directory number that the root directory only allowed is exceed
	// In other word, the new directory would not be able to create it here if this is trure
	if(directory_num > MAX_DIRS_IN_ROOT) return -EPERM;

	// We also need to check if the given directory name is already existed in the root directory
	// If not, then we will create a new dierctory file, return -EEXIST error otherwise. 
	int i;
	for( i = 0; i < directory_num; i++){
		if(strcmp(directory, root.directories[i].dname) == 0){
			// directory exist...
	...
	// Let create a new directory then update the root table
	struct cs1550_root_directory new_root = root;

	strcpy(new_root.directories[directory_num].dname, directory);
	long block_location = last_blocks * sizeof(struct cs1550_root_directory );
	new_root.directories[directory_num].nStartBlock = block_location;		
	new_root.lastAllocatedBlock += 1;
	new_root.nDirectories += 1;

	write_root(&new_root);		// Update the root directory

	// Debugging Message... to make sure the root table updated!
	struct cs1550_root_directory _root = read_root();
	printf("\nAfter write to root directory, %s..\ncurrent offset = %ld.\n", _root.directories[directory_num].dname, _root.directories[dir_numbers].nStartBlock);

	// As well, we need to create a directory entry which could have multiple files.
	struct cs1550_directory_entry new_entry;

	// Set everything inside this data field to zeros.
	memset(&new_entry, 0, sizeof(struct cs1550_directory_entry));

	new_entry.nFiles = 0;
	strcpy(new_entry.files[0].fname, "");
	strcpy(new_entry.files[0].fext, "");
	new_entry.files[0].fsize = 0;
	new_entry.files[0].nIndexBlock = 0;

	write_dir_entry( block_location, &new_entry);	// Write it on the disk where the last block created.
...
static int cs1550_mknod(const char *path, mode_t mode, dev_t dev)
{
	...
	int total_directories = root.nDirectories;			// total number of directories
	long last_block_number = root.lastAllocatedBlock;	// the offset or previous block location number
	long _offset = 0;

	// Searching through the directory in the root directory table
	int i;
	...
	// At this point the directory is trying to create in the root directory so return -EPERM error
	printf("\nDirectory not found...\n");
	return -EPERM;

	// directory_found:		// At this point there is a subdirectory that can crate a file...

	// 	// We need to update the root total directory entry because file is also would count as directory entry
	// 	// As well, we need to update on the meta data on the block location number 
	// 	root.lastAllocatedBlock = last_block_number++;

	// 	// At this point, we also need to update the directory entry table as well with the new 
	// 	// file information
	// 	struct cs1550_directory_entry entry = read_dir_entry(_offset);
	// 	int total_files_number = entry.nFiles;			// total number of files currently
	// 	int curr_file_size = (int) entry.files[total_files_number].fsize;

	// 	// Now we need to look inside the directory entry (subdirectory)
	...
	// 	// At this point, there is no existance file of user
	// 	entry.nFiles += 1;
	// 	strcpy(entry.files[total_files_number].fname, filename);
	// 	strcpy(entry.files[total_files_number].fext, extension);
	// 	entry.files[total_files_number].fsize += 1;
	// 	long next_index_block = last_block_number * sizeof(struct cs1550_directory_entry);
	// 	entry.files[total_files_number].nIndexBlock = next_index_block;


	// 	struct cs1550_disk_block new_file;
	// 	memset(&new_file, 0, sizeof(struct cs1550_disk_block));
	// 	strcpy(new_file.data, 0);

	// 	// Now, we need to add the actural file information such as index block table
	// 	struct cs1550_index_block new_index;
	// 	new_index.entries[curr_file_size] = next_index_block + (sizeof(struct cs1550_disk_block) + sizeof(struct cs1550_index_block));
...
{
	// // Check if the root directory file exit
	// struct cs1550_root_directory root;

	// root.nDirectories = 0;
	// root.lastAllocatedBlock = 1;
	// strcpy(root.directories[0].dname, "");
	// root.directories[0].nStartBlock = 0;
	// write_root(&root);
	// struct cs1550_root_directory _root  = read_root();
	// printf("Initial root values::\nLast block num = %ld\nNumDirectory = %d\n"
	// 		"Directory Name = %s\n NStartBlock = %ld\n", _root.lastAllocatedBlock, _root.nDirectories, _root.directories[0].dname, _root.directories[0].nStartBlock);
